...
import { Search, Cpu, ArrowLeft, RefreshCw, X } from 'lucide-react';
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200/80 dark:border-zinc-800/80 pb-5">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-xs font-semibold text-zinc-500 dark:text-zinc-400">
              <Link href="/" className="inline-flex items-center gap-1.5 hover:text-pink-600 dark:hover:text-pink-400 transition-colors">
                <ArrowLeft className="h-3.5 w-3.5" />
                Home
              </Link>
              <span>/</span>
              <span className="text-zinc-900 dark:text-zinc-200 font-medium">IoT Hardware</span>
            </div>
            <div className="flex items-center gap-3 pt-1">
              ...
              {!isLoading && (
                <span className="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-pink-100 text-pink-700 dark:bg-pink-950/60 dark:text-pink-300 border border-pink-200 dark:border-pink-800/50">
          ...
          <div className="w-full sm:w-72">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-8 py-2 bg-white dark:bg-[#12131a] border border-zinc-200 dark:border-zinc-800 rounded-xl text-xs outline-none focus:border-pink-500 focus:ring-1 focus:ring-pink-500 transition shadow-sm text-zinc-900 dark:text-zinc-100"
              />
              {searchQuery && (
                <button
        ...
        {isLoading ? (
          ...
          <div className="py-20 text-center bg-white dark:bg-[#12131a] border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl space-y-4 shadow-sm">
            ...
            <div className="space-y-1">
              <h3 className="text-base font-bold text-zinc-900 dark:text-white">No IoT Products Found</h3>
              <p className="text-xs text-zinc-500 max-w-sm mx-auto">
                {searchQuery ? `No hardware found matching "${searchQuery}".` : 'There are currently no IoT products available.'}
              </p>
            </div>
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="px-4 py-2 bg-pink-600 hover:bg-pink-700 text-white rounded-xl text-xs font-semibold transition"
              >
                Clear Search Filter
              </button>
            )}
