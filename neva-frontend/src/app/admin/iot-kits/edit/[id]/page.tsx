...
export default function EditIotKitPage() {
  ...
  // Existing Main Images (from DB)
  const [existingImages, setExistingImages] = useState<{ id: string; imageUrl: string }[]>([]);
  // New Main Images
  const [images, setImages] = useState<File[]>([]);

  // What's Included
  const [whatsIncluded, setWhatsIncluded] = useState<{ name: string; quantity: string; imageFile: File | null; image: string }[]>([]);
  ...
  const fetchKit = async () => {
    try {
      ...
      if (res.success && res.data) {
        ...
        setDiscountPrice(kit.discountPrice ? kit.discountPrice.toString() : '');
        setStock(kit.stock.toString());
        setStatus(kit.status);
        
        setExistingImages(kit.images || []);
        
        const mappedWhatsIncluded = (kit.whatsIncluded || []).map((w: any) => ({
          name: w.name,
          quantity: w.quantity,
          imageFile: null,
          image: w.image || ''
        }));
        setWhatsIncluded(mappedWhatsIncluded.length > 0 ? mappedWhatsIncluded : [{ name: '', quantity: '', imageFile: null, image: '' }]);
        
        setProjects(kit.projects && kit.projects.length > 0 ? kit.projects : ['']);
        setFaqs(kit.faqs && kit.faqs.length > 0 ? kit.faqs : [{ question: '', answer: '' }]);
      }
    } catch (error: any) {
      showToast('Failed to load IoT Kit details');
      console.error(error);
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const parsedWhatsIncluded = whatsIncluded.map(item => ({
        ...
        image: item.image // preserve existing image URL, backend will overwrite if new file present
      }));
      ...
      if (response.success) {
        showToast('IoT Kit updated successfully!');
        setTimeout(() => {
          router.push('/admin/iot-kits');
        }, 1500);
      }
    } catch (error: any) {
      console.error(error);
      const msg = error.message || 'Error updating IoT Kit';
      showToast(msg);
    } finally {
      setLoading(false);
    }
  ...
  if (initialLoading) {
    return <div className="p-8 flex justify-center"><div className="animate-spin h-8 w-8 border-4 border-violet-600 border-t-transparent rounded-full"></div></div>;
  }
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-20">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black tracking-tight text-zinc-900">Edit IoT Kit</h1>
          <p className="text-sm text-zinc-500 mt-1">Update kit information, images, and components</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input type="text" value={name} onChange={e => setName(e.target.value)} required className="w-full px-4 py-2 bg-zinc-50 border border-zinc-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500 text-sm" />
            </div>
            <div className="col-span-2">
              <label className="block text-xs font-semibold text-zinc-600 mb-1">Description *</label>
              <textarea value={description} onChange={e => setDescription(e.target.value)} required rows={3} className="w-full px-4 py-2 bg-zinc-50 border border-zinc-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500 text-sm"></textarea>
            </div>
            <div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm space-y-4">
          <div className="flex items-center justify-between mb-4 border-b border-zinc-100 pb-2">
            ...
            <span className="text-xs text-zinc-500">{existingImages.length + images.length}/5 uploaded</span>
          </div>
          
          <div className="flex flex-wrap gap-4">
            {/* Existing Images */}
            {existingImages.map((img, idx) => (
              <div key={`exist-${idx}`} className="relative w-24 h-24 rounded-xl border border-zinc-200 overflow-hidden group">
                <img src={img.imageUrl} alt="preview" className="w-full h-full object-cover" />
                <button type="button" onClick={() => setExistingImages(existingImages.filter((_, i) => i !== idx))} className="absolute top-1 right-1 bg-white/80 p-1 rounded-md opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            {/* New Images */}
            {images.map((img, idx) => (
              <div key={`new-${idx}`} className="relative w-24 h-24 rounded-xl border border-zinc-200 overflow-hidden group">
        ...
        <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm space-y-4">
          <div className="flex items-center justify-between mb-4 border-b border-zinc-100 pb-2">
            ...
            <button type="button" onClick={() => setWhatsIncluded([...whatsIncluded, { name: '', quantity: '', imageFile: null, image: '' }])} className="text-xs font-bold text-violet-600 hover:text-violet-700 flex items-center gap-1">
