...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 z-50 w-full transition-all duration-300 ${
      isScrolled 
        ? 'bg-zinc-950/80 backdrop-blur-md border-b border-zinc-800/50 shadow-lg' 
      ...
      <div className="mx-auto flex max-w-7xl h-16 items-center   justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-1 justify-start items-center">
          <Link href="/" className="text-xl font-black tracking-widest text-white hover:opacity-90">
        ...
        <div className="flex flex-1 justify-end items-center gap-2">
          ...
          <Link href="/cart" className="relative p-2 text-zinc-400 hover:text-white transition-colors" aria-label="Shopping Cart">
          ...
          <Link href="/login" className="hidden md:block p-2 text-zinc-400 hover:text-white transition-colors" aria-label="Profile">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden border-t border-zinc-900 bg-zinc-950 px-4 py-4 space-y-3 shadow-inner transition-all duration-200">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              onClick={() => setIsMobileMenuOpen(false)}
              className="block text-sm font-medium text-zinc-400 hover:text-white py-1.5 border-b border-zinc-900/50"
            >
              {link.label}
            </Link>
          ))}
